...
class GetUserToken: ObservableObject {
    ...
    struct StaticInstance {
        static var instance: GetUserToken?
    }
    
    class func sharedInstance() -> GetUserToken {
        if(StaticInstance.instance == nil) {
            StaticInstance.instance = GetUserToken()
        }
        return StaticInstance.instance!
    }
}

func requestGetUserToken(ID: String, PW: String) {
        // [URL 지정 및 파라미터 값 지정 실시]
        var urlComponents = URLComponents(string: "http://43.200.203.0:8080/get_user_info?")
        ...
        let dataTask = URLSession.shared.dataTask(with: requestURL) { (data, response, error) in
            ...
            GetUserToken.sharedInstance().token = responseJson["userToken"].string ?? "none"
